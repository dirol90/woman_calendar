...
import com.vio.calendar.utils.FileUtils;
...
public class PeriodicalDatabase {
    ...
    public static class DayEntry {
         ...
         * @param intensity  Intensity of the period_predicted (1-4)
         */
...
////        removePeriodYearPeriodFromLastDayOfCycle(date);
//        // Add the selected date
//    }


    @SuppressLint("DefaultLocale")
    public void addPeriodCustom(GregorianCalendar date1, GregorianCalendar date2) {
        ...
        do {
            iterator++;
            i--;
            GregorianCalendar a = (GregorianCalendar) date1.clone();
            a.add(GregorianCalendar.DATE, iterator);

            if (iterator == 0) {
                String statement;
                String datestring = format(Locale.getDefault(), "%04d%02d%02d",
                        a.get(GregorianCalendar.YEAR),
                        a.get(GregorianCalendar.MONTH)+1,
                        a.get(GregorianCalendar.DAY_OF_MONTH));

                int type = DayEntry.PERIOD_START;
                // Continue existing period_predicted to the past
                db.beginTransaction();
                ...
                statement = format(
                        ...
                        datestring);
                db.execSQL(statement);
                System.out.println("STATEMENT WRITE A"+ i+" "+ statement);
                ...
                statement = format(
                        ...
                        type);
                db.execSQL(statement);
                System.out.println("STATEMENT WRITE B"+ i+" "+ statement);
                db.setTransactionSuccessful();
                db.endTransaction();
            } else {
                String statement;
                ...
                int type = DayEntry.PERIOD_CONFIRMED;
                // Continue existing period_predicted to the past
                db.beginTransaction();
                ...
                statement = format(
                        ...
                        datestring);
                db.execSQL(statement);
                System.out.println("STATEMENT WRITE C"+ i+" "+ statement);
                statement = format(
                        "insert into data (eventdate, eventtype, intensity) values ('%s', %d, 2)",
                        datestring,
                        type);
                db.execSQL(statement);
                System.out.println("STATEMENT WRITE E" + i+" "+ statement);
...
//    }
    ...
    //    !!! HERE
    public void loadCalculatedDataCustomMethod(int cycle, int period) {
        Cursor result;
        ...
        GregorianCalendar eventdateLast = null;
        GregorianCalendar eventdateEndLast = null;
        ...
        while (result.moveToNext()) {
            ...
            switch (eventtype) {
                case DayEntry.PERIOD_START: {
                    if (eventdateLast == null) {
                        eventdateLast = (GregorianCalendar) eventdate.clone();
                    } else if (eventdate.getTimeInMillis() > eventdateLast.getTimeInMillis()) {
                        eventdateLast = (GregorianCalendar) eventdate.clone();
                    }
                }
                case DayEntry.PERIOD_CONFIRMED: {
                    if (eventdateEndLast == null) {
                        eventdateEndLast = (GregorianCalendar) eventdate.clone();
                    } else if (eventdate.getTimeInMillis() > eventdateEndLast.getTimeInMillis()) {
                        eventdateEndLast = (GregorianCalendar) eventdate.clone();
                    }
                }
            }
        ...
        System.out.println("statement Y"+eventdateLast.toString() );


        for (int i = 0; i < period; i++) {

            GregorianCalendar datePredicted = (GregorianCalendar) eventdateLast.clone();
            datePredicted.add(GregorianCalendar.DAY_OF_MONTH, cycle + i);

            DayEntry entryCalculated = new DayEntry(DayEntry.PERIOD_PREDICTED, datePredicted, 1, 1);
            dayEntries.add(entryCalculated);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            GregorianCalendar datePredicted = (GregorianCalendar) eventdateLast.clone();
            datePredicted.add(GregorianCalendar.DAY_OF_MONTH, cycle - 14 - i);

            DayEntry entryCalculated = new DayEntry(DayEntry.OVULATION_PREDICTED, datePredicted, 1, 1);
            dayEntries.add(entryCalculated);
        ...
        for (int i = 0 ; i < 45; i++){
            eventdateEndLast.add(GregorianCalendar.DAY_OF_MONTH, 1);
            statement = format(
                    "update data set eventtype=%d where eventdate='%s'",
                    DayEntry.EMPTY,
                    format(Locale.getDefault(), "%04d%02d%02d",
                            eventdateEndLast.get(GregorianCalendar.YEAR),
                            eventdateEndLast.get(GregorianCalendar.MONTH) + 1,
                            eventdateEndLast.get(GregorianCalendar.DAY_OF_MONTH)));
            db.execSQL(statement);
        }
        ...
        for (int i = 0; i < dayEntries.size(); i++) {
            ...
            statement = format(
                    "delete from data where eventdate = '%s'",
                    format(Locale.getDefault(), "%04d%02d%02d",
                            dayEntries.get(i).date.get(GregorianCalendar.YEAR),
                            dayEntries.get(i).date.get(GregorianCalendar.MONTH) + 1,
                            dayEntries.get(i).date.get(GregorianCalendar.DAY_OF_MONTH)));
            db.execSQL(statement);
            System.out.println("STATEMENT WRITE ASSS"+ i+" "+ statement);
            ...
            statement = format(
                    ...
                    format(Locale.getDefault(), "%04d%02d%02d",
                            ...
                            dayEntries.get(i).date.get(GregorianCalendar.MONTH) + 1,
                            dayEntries.get(i).date.get(GregorianCalendar.DAY_OF_MONTH)),
                    dayEntries.get(i).type);
            System.out.println("statement Z" + i + " " + statement);
            db.execSQL(statement);
        }
    ...
    public void collectEntries() {
        ...
        while (result.moveToNext()) {
            ...
            DayEntry entry = new DayEntry(eventtype, eventdate, 1, intensity);
...
//                    && entry.date.get(GregorianCalendar.DAY_OF_MONTH) == day) {
//                return entry;
//            }
//        }
//
...
//    }
    ...
    public DayEntry getEntry(GregorianCalendar date) {
        for (DayEntry entry : dayEntries) {
            // If entry was found, then return entry
            if (entry.date.get(GregorianCalendar.YEAR) == date.get(GregorianCalendar.YEAR) &&
                    entry.date.get(GregorianCalendar.MONTH) == date.get(GregorianCalendar.MONTH) &&
                    entry.date.get(GregorianCalendar.DAY_OF_MONTH) == date.get(GregorianCalendar.DAY_OF_MONTH)) {
                return entry;
            }
        }
